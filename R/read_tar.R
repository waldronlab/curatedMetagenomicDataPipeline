...
#' in each directory into a matrix. A \code{\link[Matrix]{dgTMatrix-class}}
#' matrix will be returned from the `genefamilies_relab` directory and a
#' \code{\link[base]{matrix}} will be returned in all other cases. Internally,
#' `tsv` file(s) are read in parallel using the \code{\link{furrr}} package and
#' \code{\link[readr]{read_tsv}}. When reading from the `metaphlan_bugs_list`
#' directory, the NCBI Taxonomy ID column is skipped, and, if present, the
#' `metadata` directory is ignored.
#'
#' @param tar_file A path to a tar file generated by the high load pipeline.
#'
#' @return A \code{\link[base]{list}} of \code{\link[Matrix]{dgTMatrix-class}}
#' matrix and/or \code{\link[base]{matrix}} elements.
#' @export
#'
#' @importFrom magrittr %>%
...
read_tar <- function(tar_file) {
    ...
    base::file.path(temp_dir, dir_name) %>%
        ...
        purrr::map(~ Matrix::as.matrix(.x)) %>%
        purrr::map_at("genefamilies_relab", ~ methods::as(.x, "dgTMatrix"))
}
